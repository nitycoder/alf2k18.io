...
<h1 style="color:white;font-weight:bolder"><?php
  $query = "SELECT * FROM `categories` WHERE Category = 'Cultural' LIMIT 1";
...
<?php
  $query = "SELECT * FROM `categories` WHERE Category = 'Cultural'";
  if($result=mysqli_query($link,$query)){
  while($row1 = mysqli_fetch_array($result)){
      $SubCategory = $row1['SubCategory'];
      $Image = $row1['Image'];
      echo '<div class="card card-4 view hm-zoom" style="background-color:#2E0D3A"><a href="event1.php?' .$row1['id']. '"><div><img src="img/'.$Image.'" height="100%" width="100%"/></div></br><div><h3 class="text1">'.$SubCategory.'</h3></div></a></div>';
    }
  }
?>
